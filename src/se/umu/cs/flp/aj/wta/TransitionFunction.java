...
public class TransitionFunction {
	...
	private HashMap<State, ArrayList<Rule>> rulesByResultingState =
			new HashMap<>();
	private HashMap<State, ArrayList<Rule>> rulesByState = new HashMap<>();
	...
	public ArrayList<Rule> getRulesByState(State state) {
		...
		ArrayList<Rule> rules = rulesByState.get(state);
	...
	public boolean addRule(Rule rule) throws DuplicateRuleException {
		...
		ArrayList<Rule> ruleResListState = rulesByResultingState.get(resState);
		...
		if (ruleResListState == null) {
			ruleResListState = new ArrayList<Rule>();
		...
		for (State state : rule.getStates()) {
			ArrayList<Rule> ruleListState = rulesByState.get(state);
